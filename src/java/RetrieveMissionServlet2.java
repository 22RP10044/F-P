...
public class RetrieveMissionServlet2 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<style>");
            out.println(".scrollable-form-container { max-height: 500px; overflow-y: auto; padding: 1px; border: 1px solid #ccc; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); margin: 5px auto; }");
            out.println("table { width: 1%; border-collapse: collapse; }");
            out.println("table, th, td { border: 1px solid black; }");
            out.println("th, td { padding:0px; text-align: left; }");
            out.println(".actions { display: flex; justify-content: space-between; }");
            out.println(".edit-link { color: green; font-weight: bold; text-decoration: none; margin-right: 1px; }");
            out.println(".delete-link { color: red; font-weight: bold; text-decoration: none; }");
            out.println("</style>");
            out.println("<script>");
            out.println("function confirmDelete(missionName) {");
            out.println("    return confirm('Are you sure you want to delete the mission: ' + missionName + '?');");
            out.println("}");
            out.println("</script>");
            out.println("</head>");
            ...
            out.println("<span>Mission order</span>");
            out.println("</a>");
            ...
            out.println("<a class='collapse-item' href='RetrieveMissionServlet2'>all</a>");
            out.println("<a class='collapse-item' href='RetrieveMissionServlet2'>Procesed</a>");
            out.println("</div>");
            out.println("</div>");
            out.println("</li>");
            out.println("</ul>");
            ...
            out.println("<span class='mr-2 d-none d-lg-inline text-gray-600 small'>&nbsp;&nbsp;<img class='img-profile rounded-circle' src='uploadeddata/1.png'></span>");
            out.println("</a>");
            ...
            try (Connection con = DBUtil.getConnection()) {
                ...
                try (PreparedStatement ps = con.prepareStatement(sql);
                    ...
                    while (rs.next()) {
                        String missionName = rs.getString("Purpose_of_mission");
                        out.println("<tr>");
                        ...
                        out.println("<a href='UpdateMissionServlet?id=" + rs.getInt("Id") + "' class='edit-link'>Edit</a>");
                       out.println("<a href='DeleteMissionServlet?id=" + rs.getInt("Id") + "' onclick=\"return confirmDelete('" + missionName + "')\" class='delete-link'>Delete</a>");
                        out.println("</td>");
                        out.println("</tr>");
                    }
                }
